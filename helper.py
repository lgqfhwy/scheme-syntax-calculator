...
def main(fn):
    """ 
    Call fn with command line argument. Used as a decorator.
    The main decorator marks the function that starts a program.
    For example:
    @main
    def my_run_function():
        #function body
...
_PREFIX = ''
def trace(fn):
    """ A decorator that prints a function's name, it's arguments, and its return
    values each time the function is called. For example,
    @trace
    defc ompute_something(x, y):
        # function body
    """
    @functools.wraps(fn)
    def wrapped(*args, **kwds):
        ...
        log('{0}({1})'.format(fn.__name__, ', '.join(repr)) + ':')
        _PREFIX += '     '
        try:
            result = fn(*args, **kwds)
            _PREFIX = _PREFIX[:-4]
